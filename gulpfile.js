...
gulp.task('hello', function(x) {
    console.log(x);
    console.log('Hello Lokesh Devnani!');
})
...
gulp.task('themifyAll', function(){
    ...
    colorList 
        .filter(color => color)
        .reduce(function(promise, color, i) {
...
function generateTheme(color, themeNumber) {
  return new Promise(function(resolve, reject) {
    ...
    themeNumber = themeNumber || colorValue;
    console.log("Generating Theme for ", colorValue);
    fs.writeFileSync('app/sass/_settings.scss', "$primaryColor: " + colorValue +";"  );
    gulp.src('app/sass/**/*.scss') // Gets all files ending with .scss in app/scss and children dirs
        .pipe(sass()) // Passes it through a gulp-sass
        .pipe(gulp.dest('app/css')) // Outputs it in the css folder
        .pipe(browserSync.reload({ // Reloading with Browser Sync
        stream: true
    ...
    gulp.src('app/index.html')
      ...
      .pipe(tap( function(file,t){
          file.pipe(fs.createWriteStream('dist/css/custom.'+ themeNumber +'.css', file));
      }))
      .on('end', function(){
        resolve()
      })
